...
/*==================Table Rendez-vous=============*/
CREATE TABLE appointments(id SERIAL PRIMARY KEY,patient_id INT REFERENCES users(id) ON DELETE CASCADE,docteur_id INT REFERENCES docteur(id) ON DELETE CASCADE, date TIMESTAMP NOT NULL, status VARCHAR(20) DEFAULT 'En attente', medical_condition VARCHAR(255), medical_description TEXT);
/*==================Table NOTIFICATIONS=============*/
...
/*==================Table Admin=============*/
CREATE TABLE admin(id SERIAL PRIMARY KEY, user_id INT UNIQUE REFERENCES users(id) ON DELETE CASCADE);
