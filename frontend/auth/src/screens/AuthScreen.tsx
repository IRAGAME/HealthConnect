...
export default function AuthScreen({ userRole, onSuccessfulLogin }: AuthScreenProps) {
  ...
  // Determine role: prefer prop, fall back to value saved by RoleSelectionScreen
  const currentRole: 'patient' | 'professional' = (userRole as any) || (localStorage.getItem('selectedRole') as any) || 'patient';
  ...
  const getRegisterRoute = () => {
    return currentRole === 'professional' ? '/api/auth/register/professional' : '/api/auth/register/patient';
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000${getApiRoute()}`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        // Rediriger vers la sélection d'hôpital après connexion réussie
      ...
      setError("Impossible de se connecter au serveur. Vérifiez qu'il est bien lancé.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000${getRegisterRoute()}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          nom: signupName,
  ...
  const roleTitle = userRole === 'patient' ? 'Patient' : 'Professionnel de Santé';
      ...
      <div className="w-full max-w-6xl grid md:grid-cols-2 gap-8 items-center">
        ...
        <Card className="shadow-2xl border-0">
          <CardHeader className="space-y-1 pb-6">
            <CardTitle className="text-2xl text-center">Connexion {roleTitle}</CardTitle>
